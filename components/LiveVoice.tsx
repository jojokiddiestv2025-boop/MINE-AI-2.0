...
const LiveVoice: React.FC<LiveVoiceProps> = ({ onHome, isAcademic = false }) => {
  ...
  return (
    <div className="flex flex-col flex-1 w-full max-w-[2400px] mx-auto animate-apex overflow-hidden">
      <div className="flex flex-col lg:flex-row flex-1 p-4 md:p-10 lg:p-16 gap-8 lg:gap-14 items-stretch overflow-hidden">
              ...
              {visualContext ? (
                <div className="relative group w-full h-full overflow-hidden rounded-[3.5rem]">
                  ...
                  <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center backdrop-blur-sm">
                    <button onClick={() => setVisualContext(null)} className="bg-red-600/90 text-white rounded-full p-8 hover:scale-110 active:scale-90 transition-all shadow-[0_0_80px_rgba(220,38,38,0.4)]">
                      <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" strokeWidth={3} /></svg>
                    </button>
                  </div>
                </div>
              ) : (
                <button onClick={() => fileInputRef.current?.click()} className="w-full h-full border border-white/10 rounded-[3.5rem] flex flex-col items-center justify-center text-gray-500 hover:border-blue-500/40 hover:text-white transition-all group bg-white/[0.01] shadow-2xl">
        ...
        <div className={`flex flex-col gap-8 transition-all duration-1000 order-1 lg:order-2 ${workspace.isActive ? 'w-full lg:w-[600px] xl:w-[750px]' : 'w-full lg:flex-1'}`}>
          <div className="glass-premium rounded-[5rem] p-12 md:p-24 flex-1 flex flex-col justify-center items-center relative overflow-hidden shadow-[0_80px_160px_rgba(0,0,0,0.9)] min-h-[500px]">
            {!isConnected && !isConnecting && !error && (
              <div className="text-center space-y-20 animate-apex">
                <div className="w-48 h-48 md:w-64 md:h-64 rounded-full border border-white/5 flex items-center justify-center mx-auto bg-black/40 shadow-[inset_0_0_100px_rgba(255,255,255,0.02)] relative group cursor-pointer" onClick={startConversation}>
        ...
        {workspace.isActive && (
          <div className="w-full lg:flex-1 glass-premium rounded-[4rem] animate-apex overflow-hidden flex flex-col min-h-[600px] lg:min-h-0 shadow-[0_100px_200px_rgba(0,0,0,1)] border-white/10 order-3">
            <header className="p-10 md:p-14 border-b border-white/5 flex flex-col sm:flex-row items-center justify-between bg-white/[0.01] gap-8">
            ...
            <div className="flex-1 p-10 md:p-20 overflow-y-auto custom-scrollbar">
              <div className="max-w-5xl mx-auto prose prose-invert prose-2xl prose-blue">
                <div className="whitespace-pre-wrap font-sans leading-tight text-3xl font-medium tracking-tighter text-white/90">
                  {workspace.content}
                </div>
              </div>
            </div>
