    ...
    const handleRedirect = async () => {
      try {
        ...
        if (err.code === 'auth/credential-already-in-use') {
          ...
          setError('Google authentication failed during redirect. Please try again.');
        }
      }
    };
    handleRedirect();
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!isResetPassword && (
              <div>
                <div className="flex justify-between items-center mb-2 px-1">
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500">Security Key</label>
                  {isLogin && (
                    <button 
                      type="button"
            ...
            >
              {isLoading ? (
                ...
                isResetPassword ? 'Send Reset Link' : (isLogin ? 'Establish Link' : 'Initialize Identity')
              )}
            </button>
          </form>
        </div>
        ...
        <div className="mt-8 text-center relative z-10 flex flex-col gap-3">
          {isResetPassword ? (
            <button
              ...
              className="text-gray-400 hover:text-white text-xs font-semibold transition-colors flex items-center justify-center gap-2"
            >
