...
const Auth: React.FC<AuthProps> = ({ onBack, onComplete, errorOverride }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError(err.message || 'Authentication Link Failed.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isResetPassword) {
        ...
        setSuccess('Recovery link dispatched to gateway.');
        return;
      }
      ...
      if (err.code === 'auth/invalid-credential') setError("ACCESS DENIED: Credentials mismatch.");
      else setError(err.message || 'Gateway Error.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full max-w-2xl glass-premium p-16 md:p-24 rounded-[5rem] shadow-[0_80px_150px_rgba(0,0,0,0.08)] relative border-white overflow-hidden bg-white/90">
        ...
        <div className="flex flex-col items-center mb-16 text-center relative z-10 pt-4">
          ...
          <h2 className="text-5xl md:text-6xl font-outfit font-black tracking-tight text-slate-900 uppercase mt-10">
            {isResetPassword ? 'Reset Key' : (isLogin ? 'Personal Node' : 'Initialize Node')}
          </h2>
        ...
        <div className="space-y-10 relative z-10">
          {!isResetPassword && (
            <button onClick={handleGoogleSignIn} disabled={isLoading} className="w-full py-6 rounded-[2.5rem] border-2 border-slate-100 bg-white flex items-center justify-center gap-6 text-[12px] font-black uppercase tracking-[0.5em] text-slate-900 hover:bg-slate-50 transition-all shadow-xl active:scale-95 group">
              ...
              <span>{isLogin ? 'Sync via Google' : 'Provision via Google'}</span>
            </button>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-10">Encryption Identity</label>
              <input type="email" value={email} onChange={(e) => setEmail((e.target as any).value)} className="w-full bg-slate-50/50 border border-slate-100 rounded-[2.5rem] px-10 py-6 text-slate-900 font-bold text-xl outline-none focus:ring-4 focus:ring-accent/10 transition-all" placeholder="node@nexus.ai" required />
            </div>

            {!isResetPassword && (
              <div className="space-y-4">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-10">Access Key</label>
            ...
            <button type="submit" disabled={isLoading} className="button-billion w-full !py-8 text-lg shadow-2xl active:scale-95 flex items-center justify-center gap-6">
              {isLoading ? <div className="w-6 h-6 border-3 border-white/30 border-t-white rounded-full animate-spin"></div> : <span>Unlock Node</span>}
            </button>
          </form>
        </div>
        ...
        <div className="mt-16 text-center relative z-10 flex flex-col gap-8">
          <button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-accent hover:text-slate-900 text-[11px] font-black transition-colors uppercase tracking-[0.6em]">
            {isLogin ? "Need a Personal Node?" : "Return to Gateway"}
          </button>
          {!isResetPassword && (
