...
const Auth: React.FC<AuthProps> = ({ mode, isRegisteringInstitution, onBack, onComplete, errorOverride }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (mode === 'school') {
        // Find if this Google account is linked to an admin or a provisioned member
        const allSchools: string[] = Object.keys(localStorage).filter(k => k.startsWith('mine_school_data_'));
        let foundRole = '';
        ...
        if (!foundRole) {
          throw new Error("IDENTITY NOT IN REGISTRY: This Google account is not recognized by any institution.");
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        if (mode === 'school') {
          if (isAdminLogin) {
            if (!schoolData) throw new Error("ACCESS BLOCKED: Admin identity not found.");
          } else {
            if (!provisionedMember) throw new Error("IDENTITY NOT FOUND: Node not provisioned.");
          }
        }
      ...
      setError(err.message || 'Handshake Protocol Failure.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-6 animate-billion relative bg-slate-50 overflow-hidden">
      ...
      <div className="w-full max-w-2xl glass-premium p-12 md:p-20 rounded-[4.5rem] shadow-2xl relative border-white/90 overflow-hidden bg-white/80">
        ...
        <div className="absolute top-10 left-10 flex items-center gap-3">
           ...
           <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">NEURAL SSL ACTIVE</span>
        </div>
        ...
        <div className="space-y-8 relative z-10">
          {!isResetPassword && (
            <button onClick={handleGoogleSignIn} disabled={isLoading} className="w-full py-5 rounded-[2rem] border-2 border-slate-900/5 bg-white flex items-center justify-center gap-4 text-[11px] font-black uppercase tracking-[0.4em] text-slate-900 hover:bg-slate-50 transition-all shadow-xl active:scale-95 group">
              ...
              <span>{isLogin ? 'Sync with Google' : 'provision with google'}</span>
            </button>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {showIdentityField ? (
              ...
              <div className="space-y-3">
                <label className="text-[8px] font-black uppercase tracking-widest text-slate-400 px-8">AUTHORIZED ADDRESS (EMAIL)</label>
            ...
            <button type="submit" disabled={isLoading} className="button-billion w-full !py-6 text-sm shadow-2xl active:scale-95 flex items-center justify-center gap-4">
              {isLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <span>VERIFY HANDSHAKE</span>}
            </button>
          </form>
        </div>
        ...
        <div className="mt-12 text-center relative z-10 flex flex-col gap-6">
          {!isRegisteringInstitution && !isResetPassword && (
            <button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-slate-400 hover:text-cyan-600 text-[9px] font-black transition-colors uppercase tracking-[0.5em]">
              {isLogin ? "provision new neural node?" : "Return to login"}
            </button>
          )}
          {mode === 'school' && !isRegisteringInstitution && (
            <button onClick={() => setIsAdminLogin(!isAdminLogin)} className="text-[8px] font-black text-slate-300 hover:text-slate-500 uppercase tracking-[0.6em]">
              {isAdminLogin ? "back to student access" : "administrative gateway"}
            </button>
          )}
        </div>
      </div>
