...
const TextChat: React.FC<TextChatProps> = ({ userName = 'User' }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      // ATTEMPT 1: Primary Engine (Puter GPT-4o)
      try {
        ...
        // HANDOVER: Switch to Gemini 3 Pro
        console.warn("Primary Core limit reached. Initiating Neural Handover...", puterError);
        setEngineStatus('NEURAL HANDOVER IN PROGRESS...');
        setActiveEngine('Secondary');
        
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY || '' });
        const parts: any[] = [{ text: currentInput || "Analyze the provided context for a high-priority request." }];
      ...
      setMessages(prev => [...prev, {
        id: 'critical-error',
        role: 'assistant',
        content: "I've encountered a total neural blackout. Re-establishing link...",
        timestamp: Date.now(),
      }]);
      setEngineStatus('CONNECTION FAILURE');
    } finally {
      setIsTyping(false);
    }
  };
    ...
    <div className="flex flex-col h-full w-full max-w-5xl mx-auto bg-white overflow-hidden animate-billion">
      <div className="px-10 py-5 bg-slate-50 border-b border-slate-100 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-8">
           <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-300">AUTO-SWAP ACTIVE</span>
           <div className="h-4 w-[1px] bg-slate-200"></div>
           <span className="text-[9px] font-black uppercase tracking-[0.2em] text-accent">MINE V5.5 PLATINUM</span>
        </div>
      </div>
              ...
              {msg.images && msg.images.length > 0 && (
                <div className="flex flex-wrap gap-6 mb-12">
                  {msg.images.map((img, idx) => (
                    <div key={idx} className="relative group/img overflow-hidden rounded-[2.5rem] shadow-2xl border-4 border-white transition-all hover:scale-105 active:scale-95 cursor-zoom-in">
                      <img src={img} alt="Visual context" className="w-48 h-48 md:w-80 md:h-80 object-cover" />
                    </div>
                  ))}
                </div>
              )}
